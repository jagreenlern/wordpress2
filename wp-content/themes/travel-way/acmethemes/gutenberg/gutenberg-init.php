...
function travel_way_dynamic_editor_styles(){
	...
	$travel_way_link_color               = esc_attr( $travel_way_customizer_all_values['travel_way-link-color'] );
	$travel_way_link_hover_color         = esc_attr( $travel_way_customizer_all_values['travel_way-link-hover-color'] );
...
function travel_way_block_editor_styles() {
	wp_enqueue_style( 'travel_way-googleapis', '//fonts.googleapis.com/css?family=Source+Sans+Pro:200,200i,300,300i,400,400i,600,600i,700,700i,900,900i', array(), null );
	wp_enqueue_style( 'travel_way-block-editor-styles', get_template_directory_uri() . '/acmethemes/gutenberg/gutenberg-edit.css', false, '1.0' );

	/**
	 * Styles from the customizer
	 */
	wp_add_inline_style( 'travel_way-block-editor-styles', travel_way_dynamic_editor_styles() );
}
add_action( 'enqueue_block_editor_assets', 'travel_way_block_editor_styles',99 );

function travel_way_gutenberg_scripts() {
	wp_enqueue_style( 'travel_way-block-front-styles', get_template_directory_uri() . '/acmethemes/gutenberg/gutenberg-front.css', false, '1.0' );
	wp_style_add_data( 'travel_way-block-front-styles', 'rtl', 'replace' );
}
