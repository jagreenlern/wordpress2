<?php
if (!class_exists('Medical_Hub_Info_Widget')) {
    class Medical_Hub_Info_Widget extends WP_Widget
    {
        ...
        {
            parent::__construct(
                'medical_hub-info-widget',
                esc_html__('AT : info Widget', 'medical-hub'),
                array('sub_title' => esc_html__('business info Section', 'medical-hub'))
            );
        }
        ...
        public function form($instance)
        {
            ...
            foreach ($counters as $countervalue) {
                ...
                <p>
                    <label
                        for="<?php echo esc_attr($this->get_field_id('icon' . $countervalue)); ?>"><?php echo esc_html('Icon ' . $countervalue, 'medical-hub'); ?>
                    </label><br/>
                    <small>
                        <?php
                        esc_html_e('Font Awesome Icon Used in Widgets', 'medical-hub');
                        printf(__('%1$sRefer here%2$s for icon class. For example: %3$sfa-desktop%4$s', 'medical-hub'), '<br /><a href="' . esc_url('http://fontawesome.io/cheatsheet/') . '" target="_blank">', '</a>', "<code>", "</code>");
                        ?>
                    </small>
                    <strong>
                ...
                <p>
                    <label for="<?php echo esc_attr($this->get_field_id('title' . $countervalue)); ?>">
                        <?php echo esc_html('Title ' . $countervalue, 'medical-hub'); ?>
                    </label><br/>
                ...
                <p>
                    <label for="<?php echo esc_attr($this->get_field_id('sub_title' . $countervalue)); ?>">
                        <?php echo esc_html('Sub-title ' . $countervalue, 'medical-hub'); ?>
                    </label><br/>
                    <input type="text"
...
add_action('widgets_init', 'medical_hub_info_widget');
function medical_hub_info_widget()
{
    register_widget('Medical_Hub_Info_Widget');
