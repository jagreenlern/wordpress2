...
function innofit_comments( $comment, $args, $depth ){
	...
	//translations
	$leave_reply = isset($comment_data['translation_reply_to_coment']) ? $comment_data['translation_reply_to_coment'] : esc_html__('Reply','innofit');
?>
<!--Comment-->
<div <?php comment_class('media comment-box'); ?> id="comment-<?php comment_ID(); ?>">
	...
	<div class="media-body">
		<div class="comment-detail">
			...
			<span class="comment-date"><?php esc_html_e('Posted on ','innofit'); ?><?php   comment_time(); ?><?php echo " - ";  comment_date();?></span>
			<?php comment_text(); ?>
...
<?php if ('open' == $post->comment_status) :
	...
	function innofit_fields( $innofit_fields )
	{
		return $innofit_fields;
	}
	add_filter('comment_form_default_fields','innofit_fields');

	$innofit_defaults = array(

		'fields'=> apply_filters( 'comment_form_default_fields', $innofit_fields ),
