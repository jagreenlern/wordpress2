...
class innofit_featured_latest_news extends WP_Widget {
					...
					<?php if($instance['image_show']==true): if(has_post_thumbnail()):?>
						<a href="<?php the_permalink(); ?>">
							...
							if($instance['tumb_size'] == 'thumbnail') {
								$defalt_arg =array('class' => "post-thumbnail" );
								the_post_thumbnail($instance['tumb_size'], $defalt_arg);
							}
							if($instance['tumb_size'] == 'medium') {
					 			$defalt_arg =array('class' => "lates_news_medium" );
						   	the_post_thumbnail($instance['tumb_size'], $defalt_arg);
							}
							if($instance['tumb_size'] == 'large') {
							 	$defalt_arg =array('class' => "lates_news_large" );
						   	the_post_thumbnail($instance['tumb_size'], $defalt_arg);
							}
							if($instance['tumb_size'] == 'full') {
							 	$defalt_arg =array('class' => "lates_news_full" );
						   	the_post_thumbnail($instance['tumb_size'], $defalt_arg);
							}
							?>
						</a>
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance['tumb_size'] = ( ! empty( $new_instance['tumb_size'] ) ) ? sanitize_text_field( $new_instance['tumb_size'] ) : '';
		$instance['blog_cat'] = ( ! empty( $new_instance['blog_cat'] ) ) ? intval( $new_instance['blog_cat'] ) : '';
		$instance['image_show'] = ( ! empty( $new_instance['image_show'] ) ) ? innofit_sanitize_checkbox($new_instance['image_show']) : '';
