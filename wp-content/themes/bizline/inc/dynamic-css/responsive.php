<?php
add_action( 'init', 'bizline_responsive_device_css' );
 ...
 */
function bizline_responsive_device_css(){
	foreach( array( 'md' => 'desktop', 'sm' => 'tablet', 'xs' => 'mobile' ) as $size => $device ){

		Bizline_Css::add_styles( array(
			#Typography
			array(
			...
			array(
				'selector'  => Bizline_Helper::with_prefix_selector( '%s-post .post-title a' ),
				'props'		=> array(
			...
			array(
				'selector' => Bizline_Helper::with_prefix_selector( '%s-inner-banner-wrapper' ),
				'props' => array(
			...
			array(
				'selector' => 'section.bizline-get-quote-section',
				'props' => array(
					'margin-top' => 'home-quote-margin-' . $device . '-top',
					'margin-bottom' => 'home-quote-margin-' . $device . '-bottom'
				)
			),
			array(
