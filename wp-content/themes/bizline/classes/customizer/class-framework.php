...
if( !class_exists( 'Bizline_Customizer' ) ){
	class Bizline_Customizer extends Bizline_Helper{
		...
		*/
		public static function add_custom_control( $control ){
			...
			if( !is_array( $control ) ){
				$err = esc_html__( 'Invalid argument passed for Custom Control. Must be an array with "type" & "class" key.', 'bizline' );
				self::add_error( $err );
				return;
			}
			...
			if( !isset( $control[ 'type' ] ) ){
				$err = esc_html__( 'Custom Control must contain "type" key ( type of control ).', 'bizline' );
				self::add_error( $err );
				$flag = false;
			}

			if( !isset( $control[ 'class' ] ) ){
				$err = esc_html__( 'Custom Control must contain "class" key ( name of custom class ).', 'bizline' );
				self::add_error( $err );
				$flag = false;
			}		

			if( !isset( $control[ 'sanitize' ] ) ){
				$err = esc_html__( 'Custom Control must contain "sanitize" key ( a callback function ).', 'bizline' );
				self::add_error( $err );
				$flag = false;
			}
		...
		*/
		public static function add_custom_section( $section ){
			...
			if( !is_array( $section ) ){
				$err = esc_html__( 'Invalid argument passed for Custom Section. Must be an array with "type" & "class" key.', 'bizline' );
				self::add_error( $err );
				return;
			}
			...
			if( !isset( $section[ 'type' ] ) ){
				$err = esc_html__( 'Custom Section must contain "type" key ( type of section ).', 'bizline' );
				self::add_error( $err );
				$flag = false;
			}

			if( !isset( $section[ 'class' ] ) ){
				$err = esc_html__( 'Custom Section must contain "class" key ( name of section class ).', 'bizline' );
				self::add_error( $err );
				$flag = false;
			}		
				...
				foreach( $args[ 'fields' ] as $field ){
					...
					if( isset( $field[ 'id' ] ) ){
						...
						if( array_key_exists( $field_id, self::$settings ) ){
							...
							$msg = sprintf( esc_html__( 'Field with ID: %s already exists.', 'bizline' ), $field[ 'id' ] );
							self::add_error( $msg );
						}
		...
		*/
		private static function validate_argument( $args ){
			...
			if( !is_array( $args ) ){
				...
				$msg = esc_html__( 'You must pass array as an argument in set method.', 'bizline' );
			...
			if( !isset( $args[ 'section' ] ) ){
				...
				$msg = esc_html__( 'No section found.', 'bizline' );
				self::add_error( $msg, 2 );
			...
			if( !is_array( $args[ 'section' ] ) ){
				...
				$msg = sprintf( 
					...
					esc_html__( 'Invalid argument', 'bizline' ),	
					esc_html__( 'section', 'bizline' ),
					esc_html__( 'Must be a an array with atleast an id, title...', 'bizline' )
				);
				...
				$msg = esc_html__( 'Section id is missing.', 'bizline' );
				self::add_error( $msg, 2 );
			}
			...
			if( isset( $args[ 'fields' ] ) && !is_array( $args[ 'fields' ] ) ){
				...
				$msg = sprintf( 
					'%s <strong><em>%s</em></strong>. %s',
					esc_html__( 'Invalid argument', 'bizline' ),	
					esc_html__( 'fields', 'bizline' ),
					esc_html__( 'Must be a an array', 'bizline' )
				);
						...
						<p>
							<?php  
								echo sprintf( '<strong>%s</strong> %s', 
									esc_html__( 'Bizline Customizer:', 'bizline' ),
									esc_html( $value ) 
								);  
							?>
						</p>
		...
		*/
		private static function get_panel_arg( $panel ){
			...
			$args = array(
			    ...
			    esc_html__( 'No Title Specified.', 'bizline' ) : 
			    $panel['title'],
			);
		...
		*/
		private static function get_section_arg( $panel_id = false, $section ){
			$args = array(
				'title' => empty( $section[ 'title' ] ) ? esc_html__( 'No Title Specified.', 'bizline' ) : $section[ 'title' ],
			);
...
add_action( 'customize_register', array( Bizline_Customizer::get_instance(), 'register' ) );

if( !function_exists( 'bizline_get' ) ):
	/**
	 ...
	 * @package Bizline WordPress Theme
	 */
	function bizline_get( $id ){
		$id = Bizline_Helper::with_prefix( $id );
		return Bizline_Customizer::get( $id );
	}
