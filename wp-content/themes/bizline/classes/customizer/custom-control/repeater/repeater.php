...
if ( class_exists( 'WP_Customize_Control' ) ) {
	class Bizline_Page_Repeater extends WP_Customize_Control {
		...
		public function to_json() {
			...
			$select_pages = array( esc_html__( 'Select page', 'bizline' ) ); 
		...
		protected function content_template() {
			...
			<div class="page-repeater-template hidden">
				<div>
					...
					<button data-index="{#index}" data-limit="{{ data.limit }}" class="page-repeater-remove"> 
						<span class="dashicons dashicons-trash"></span><?php esc_html_e( 'Delete', 'bizline' ) ?>
					</button>
				</div>
			</div>
			...
			<div id="repeater_{{ data.id }}">
				...
				>
					<?php esc_html_e( 'Add new', 'bizline' ); ?>
...
Bizline_Customizer::add_custom_control( array(
    'type'     => 'repeater',
    'class'    => 'Bizline_Page_Repeater',
    'sanitize' =>  array( 'Bizline_Page_Repeater', 'sanitize' )
));

function bizline_page_repeater_control( $args, $section, $field ){
...
add_filter( 'bizline_customizer_get_control_arg', 'bizline_page_repeater_control', 10, 3 );

function bizline_page_repeater_get( $value, $id, $instance ){
...
add_filter( 'bizline_customizer_get', 'bizline_page_repeater_get', 10, 3 );