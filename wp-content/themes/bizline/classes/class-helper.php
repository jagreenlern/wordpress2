 ...
 */
if( !class_exists( 'Bizline_Helper' ) ):
	class Bizline_Helper{
		...
		public static function skip_content(){
			...
			<a class="skip-link screen-reader-text" href="#content">
				<?php esc_html_e( 'Skip to content', 'bizline' ); ?>
			</a>
			<?php
		}
		...
		public static function the_header_image(){
			...
			}elseif( self::is_static_blog_page() ){												# since 1.0.2
		...
		public static function excerpt_more( $text = false ){
			if( is_admin() ){
				return $text;
			}elseif( self::is_static_front_page() ){
				return '...';
			}
		...
		*/
		...
		public static function get_comment_number(){
			if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
				echo '<span class="bizline-comments">';
				comments_popup_link(
					'<i class="fa fa-comment"></i> '.esc_html__( 'Leave a comment', 'bizline' ),		
					'<i class="fa fa-comment"></i> '.esc_html__( '1 response', 'bizline' ),
					'<i class="fa fa-comments"></i> % '. esc_html__( 'responses' , 'bizline' )
				);
				echo '</span>';
			}
		}		
		...
		*/
		public static function get_assets_version(){
			return bizline_get( 'assets-version' );
		}
		    ...
		    switch( $location ){
		        ...
		        case 'primary':
			        $menu = wp_nav_menu( array(
			        	...
			        	'container_class' =>'bizline-main-menu'
			        ));
		        break;
		...
		*/
		public static function posts_navigation( $echo = true ) {
			...
			if( !$infinity_module_active || !Bizline_Helper::is_active_plugin( 'jetpack' ) ){				
				the_posts_pagination(
					array(
						'mid_size'  => 2,
						'prev_text' => esc_html__( 'Previous', 'bizline' ),
						'next_text' => esc_html__( 'Next', 'bizline' ),
					)
				);
			}
		}
		...
		*/
		public static function post_content_navigation(){
			wp_link_pages( array(
				'before'      => '<div class="page-links">' . esc_html__( 'Pages:', 'bizline' ),
                'after'       => '</div>',
		...
		*/
		public static function single_post_navigation(){
			the_post_navigation( array(
				'prev_text' => '<span class="screen-reader-text">' . esc_html__( 'Previous Post', 'bizline' ) . '</span><span class="nav-title">%title</span>',
				'next_text' => '<span class="screen-reader-text">' . esc_html__( 'Next Post', 'bizline' ) . '</span><span class="nav-title">%title</span>',
			));
		}
		...
		*/
		 ...
		 * @package Bizline WordPress Theme
		 */
		public static function get_author_image( $author_id = false ){
		 ...
		 * @package Bizline WordPress Theme
		 */
		public static function posted_by( $author_id = false ){
			if( apply_filters( self::fn_prefix( 'show_post_author' ), true ) ):
				$author_id = $author_id ? $author_id : get_the_author_meta( 'ID' );
				$name = $author_id ? get_the_author_meta( 'nicename', $author_id ) : get_the_author();
				printf(
		...
		public static function the_date( $status = 'posted' ) {
			...
			if( $show ):
				...
				printf(
					...
					( 'posted' == $status ) ? esc_html__( 'On', 'bizline' ) : esc_html__( 'Updated on', 'bizline' ),
					$time_tag
				);
			endif;
		}
		...
		public static function the_breadcrumb(){
			...
			if( apply_filters( self::fn_prefix( 'show_breadcrumb' ), true ) ): 
			    ...
			    <div id="<?php echo esc_attr( self::with_prefix( 'breadcrumb' ) ) ?>" class="<?php echo esc_attr( $classes ); ?>">
			    	<?php bizline_breadcrumb_trail( $breadcrumb_args ); ?>
				</div><!-- #breadcrumb -->
			    <?php return;
			endif;
		}
					...
					<?php 
						if( is_page() || is_singular() ){
							...
							<h2 class="entry-title">
								<?php 
									printf( 
										esc_html__( 'Search Results for: %s', 'bizline' ), 
		...
		/**
		* Font family used in Bizline Theme
		*
		* @static
		* @access public
		* @return object
		...
		*/
		public static function get_font_family( ){
			return apply_filters( self::fn_prefix( 'standard_fonts_array' ), array(
				'font-1'  => esc_html__( 'Lato', 'bizline' ),
				'font-2'  => esc_html__( 'Oswald', 'bizline' ),
				'font-3'  => esc_html__( 'Montserrat', 'bizline' ),
				'font-4'  => esc_html__( 'Roboto', 'bizline' ),  
				'font-5'  => esc_html__( 'Raleway', 'bizline' ),  
				'font-6'  => esc_html__( 'Playfair Display', 'bizline' ),  
				'font-7'  => esc_html__( 'Fjalla One', 'bizline' ),  
				'font-8'  => esc_html__( 'Alegreya Sans', 'bizline' ),
				'font-9'  => esc_html__( 'PT Sans Narrow', 'bizline' ),
				'font-10' => esc_html__( 'Open Sans', 'bizline' ),
				'font-11' => esc_html__( 'Poppins', 'bizline' ),
				'font-12' => esc_html__( 'Hind', 'bizline' ),
			));
		}
		...
		public static function get_google_font(){
			...
			$google_fonts[] = self::uglify( self::get_font( bizline_get( 'body-font' ) ), $condition );
			$google_fonts[] = self::uglify( self::get_font( bizline_get( 'heading-font' ) ), $condition );
			$google_fonts[] = self::uglify( self::get_font( bizline_get( 'site-info-font' ) ), $condition );
			$fonts =  array_unique( $google_fonts );
			$fonts_weight = apply_filters( self::fn_prefix( 'standard_fonts_weight' ), array(
				'Montserrat' => array( '100', '200', '300', '400', '500', '600', '700', '800' ),
				'Lato' 		 => array( '100', '200', '300', '400', '500', '600', '700', '800' ),
				'Open+Sans'  => array( '100', '200', '300', '400', '500', '600', '700', '800' ),
				'Poppins' 	 => array( '100', '200', '300', '400', '500', '600', '700', '800' ),
				'Hind' 	 => array( '100', '200', '300', '400', '500', '600', '700', '800' )
			));
