<?php
/**
 * Bizline functions and definitions
 *
 * Bizline only works in WordPress 4.7 or later.
 *
...
define( 'BIZLINE_PREFIX', 'bizline' );
define( 'BIZLINE_VERSION', '1.0.8' );

require get_parent_theme_file_path( '/classes/class-helper.php' );

final class Bizline_Theme extends Bizline_Helper{
	/**
	 * constructor
	 *
	 * @since 1.0.0
	...
	public function __construct(){
		...
		# if pro child theme is activated then save all settings of parent
		add_action( 'switch_theme', array( __CLASS__, 'save_setting_for_pro' ), 10, 2 );
		...
		add_action( self::fn_prefix( 'after_primary_menu' ), array( __CLASS__, 'add_search_icon' ) );
		#woocommerce cart icon on header
		add_action( self::fn_prefix( 'after_primary_menu' ), array( __CLASS__, 'add_cart_icon' ), 5 );
		...
		add_filter( 'frontpage_template', array( __CLASS__, 'front_page_template' ) );
	}
	...
	*/
	public static function get_footer_widget(){
		...
		if( is_page() && $disable_footer ){
			...
			return true;
		}elseif( self::is_static_blog_page() && $disable_footer ){
			return true;
		}
	}
	...
	*/
	public static function customize_register( $wp_customize ){
		...
		$wp_customize->get_section( 'header_image' )->title = esc_html__( 'Inner Banner Options', 'bizline' );
	...
	*/
	public static function meta_fields(){
		...
		$label = esc_html__( 'Bizline Settings', 'bizline' );
		foreach ( $post_types as $type ) {

			$post = new Bizline_Meta_Fields( $type );
			$options = array(
				'sidebar-position' => array(
					...
					'default' => 'customizer',
					'choices' => array(
						'customizer' 	=> esc_html__( 'From customizer', 'bizline' ),
						'left-sidebar' 	=> esc_html__( 'Left', 'bizline' ),
						'right-sidebar' => esc_html__( 'Right', 'bizline' ),
						'no-sidebar' 	=> esc_html__( 'Hide', 'bizline' ),
					),
				),
			);
			...
			if( 'page' == $type ){
				$page_options = array(
					'disable-inner-banner' => array(
						'type'  => 'checkbox',
						'label' => esc_html__( 'Disable Banner', 'bizline' ),
					),
					'disable-footer-widget' => array(
						'type'  => 'checkbox',
						'label' => esc_html__( 'Disable Footer Widget', 'bizline' ),
					),
				);
	...
	*/
	...
	public static function topbar_has_content(){
		return( 
			bizline_get( 'location' ) ||
			bizline_get( 'phone-num' ) ||
			bizline_get( 'email' ) ||
			bizline_get( 'header-social-menu' )
		);
	}
	...
	*/
	public static function get_excerpt_length() {
		return bizline_get( 'excerpt_length' );
	}

	/**
	* Get excerpt length from customizer for home page slider
	*	
	* @static
	* @access public
	* @return interger
	...
	* @package Bizline WordPress Theme
	*/
	public static function get_excerpt_length_slider() {
		return bizline_get( 'home-slider-excerpt-length' );
	}
	...
	public static function get_blog_title(){
		...
		$ib_blog_title = bizline_get( 'ib-blog-title' );
	...
	public static function show_breadcrumb(){
		...
		return bizline_get( 'show-breadcrumb' );
	}
	...
	public static function show_post_author(){
		return bizline_get( 'post-author' );
	}
	...
	public static function show_post_category(){
		return bizline_get( 'post-category' );
	}
	...
	public static function supports(){
		 ...
		 */
		load_theme_textdomain( 'bizline', self::get_theme_path( 'languages' ) );
	}
	...
	public static function sidebars(){
		...
		register_sidebar( array(
	        'name' 			=> esc_html__( 'Sidebar', 'bizline' ),
	        'id' 			=> self::fn_prefix( 'sidebar' ),
	        'description' 	=> esc_html__( 'Widgets in this area will be shown on side of the page.', 'bizline' ),
	        'before_widget' => '<section id="%1$s" class="widget %2$s">',
	        'after_widget'  => '</section>',
	        'before_title'  => '<h2 class="widget-title">',
	        'after_title'   => '</h2>',
		));
		$description = esc_html__( 'Widgets in this area will be displayed in the {position} column in the footer. If empty then column will not be displayed.', 'bizline' );
		for( $i = 1; $i <= bizline_get( 'layout-footer' ); $i++ ){
			switch ($i){
				case '1':
					$position = esc_html__( 'first', 'bizline' );
				break;
				case '2':
					$position = esc_html__( 'second', 'bizline' );
				break;
				case '3':
					$position = esc_html__( 'third', 'bizline' );
				break;
				case '4':
					$position = esc_html__( 'fourth', 'bizline' );
				break;
				default:
					$position = esc_html__( 'first', 'bizline' );
			...
			register_sidebar( array(
		        ...
		        'name' 			=> sprintf( esc_html__( 'Footer Widget Area %d ', 'bizline' ), $i ),
	...
	public static function nav_menu(){
		register_nav_menus(
			array(
				'primary' => esc_html__( 'Primary', 'bizline' ),
				'social-menu-header' => esc_html__( 'Header social menu', 'bizline' )
			)
		);
	}
	...
	public static function scripts(){
		...
		$scripts = array(
		    ...
		    array(
		        ...
		        'localize'	=> array(
		        	'key'	=> 'bizline_home_page',
		        	'data'	=> array(  
		        		'admin_url'	=> esc_url( admin_url( 'admin-ajax.php' ) ),
		        		'arrow'		=> bizline_get( 'home-slider-arrow' ) ? true : false,
		        		'pager'		=> bizline_get( 'home-slider-pager' ) ? true : false,
		        	),
		        )
		    ),
		);
	...
	public static function get_sidebar_class( $classes ){
		...
		if( $page_template || is_search() || self::is_woo_single_page() || self::is_woo_product_category() ){
			...
			$customizer_position = bizline_get( 'sidebar-position' );
	...
	public static function inner_banner_class( $classes ){
		$classes[] = bizline_get( 'ib-text-align' );
		if( has_header_image() ){
			$classes[] = bizline_get( 'ib-image-attachment' );
		}
	...
	public static function get_body_classes ( $classes ){
		# Container class
		if( 'box' == bizline_get( 'container-width' ) ){
			$classes[] = self::with_prefix( 'container-box' );
		}

		#header option 

		if(bizline_get( 'transparent-header' )){
	...
	public static function register_required_plugins(){
		$plugins = array(
			array(
				'name'     => esc_html__( 'Rise Blocks - A Complete Gutenberg Page builder', 'bizline' ),
				'slug'     => 'rise-blocks',
				'required' => false,
			),
		);
	...
	*/
	 ...
	 * @package Bizline WordPress Theme
	 */
	public static function get_front_page_templates(){
		$order = apply_filters( self::fn_prefix( 'add_sections' ), array(
			'slider', 'quote', 'service', 'feature', 'testimonial', 'team', 'cta', 'news'
		));

		foreach ( $order as $o ) {
			get_template_part( 'templates/front-page/home', $o );
		}
	}
	 ...
	 * @package Bizline WordPress Theme
	 */
	public static function save_setting_for_pro( $new_name, \WP_Theme $new_theme ){
	...
	*/
	public static function footer_social_menu(){
		if( !bizline_get( 'footer-social-menu' ) ){
			return;
		}
