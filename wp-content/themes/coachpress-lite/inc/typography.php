<?php
/**
 * CoachPress Lite Typography Related Functions
 *
 * @package CoachPress_Lite
 */

if( ! function_exists( 'coachpress_lite_fonts_url' ) ):
/**
 * 
*/ 
function coachpress_lite_fonts_url(){
    ...
    */
    $primary    = _x( 'on', 'Primary Font: on or off', 'coachpress-lite' );
    $secondary  = _x( 'on', 'Secondary Font: on or off', 'coachpress-lite' );
    $tertiary   = _x( 'on', 'Tertiary Font: on or off', 'coachpress-lite' );
    $site_title = _x( 'on', 'Site Title Font: on or off', 'coachpress-lite' );
...
if( ! function_exists( 'coachpress_lite_get_google_fonts' ) ) :
/**
 * Get Google Fonts
*/
function coachpress_lite_get_google_fonts(){
...
if( ! function_exists( 'coachpress_lite_get_websafe_font' ) ) :
/**
 * Function listing WebSafe Fonts and its attributes
*/
function coachpress_lite_get_websafe_font(){
    ...
    return apply_filters( 'coachpress_lite_standard_fonts', $standard_fonts );
}
endif;

if( ! function_exists( 'coachpress_lite_check_varient' ) ) :
/**
 * Checks for matched varients in google fonts for typography fields
*/
function coachpress_lite_check_varient( $font_family = 'serif', $font_variants = 'regular', $body = false ){
    $variant = '';
    $var     = array();
    $google_fonts  = coachpress_lite_get_google_fonts(); //Google Fonts
    $websafe_fonts = coachpress_lite_get_websafe_font(); //Standard Web Safe Fonts
...
*/
function coachpress_lite_get_css_variant( $font_variant ){
    $v_array = array(
		'100'       => array(
            'weight'    => '100',
            'style'     => 'normal'
...
*/
function coachpress_lite_is_google_font( $font ){
    $return = false;
    $websafe_fonts = coachpress_lite_get_websafe_font();
    if( $font ){
...
*/
function coachpress_lite_get_fonts( $font_family, $font_variant ){
    
    $fonts = array();
    $websafe_fonts = coachpress_lite_get_websafe_font(); //Web Safe Font
    
    if( $font_family ){
        if( coachpress_lite_is_google_font( $font_family ) ){
            $fonts['font'] = esc_attr( $font_family ); //Google Font
            if( $font_variant ){
                $weight_style    = coachpress_lite_get_css_variant( coachpress_lite_check_varient( $font_family, $font_variant ) );
...
if( ! function_exists( 'coachpress_lite_get_all_fonts' ) ) :
/**
 * Return Web safe font and google font without variants
*/
function coachpress_lite_get_all_fonts(){
    $google = array();        
    $standard = apply_filters( 'coachpress_lite_standard_font', array(
		'georgia-serif'       => __( 'Georgia', 'coachpress-lite' ),
        'palatino-serif'      => __( 'Palatino Linotype, Book Antiqua, Palatino', 'coachpress-lite' ),
        'times-serif'         => __( 'Times New Roman, Times', 'coachpress-lite' ),
        'arial-helvetica'     => __( 'Arial, Helvetica', 'coachpress-lite' ),
        'arial-gadget'        => __( 'Arial Black, Gadget', 'coachpress-lite' ),
		'comic-cursive'       => __( 'Comic Sans MS, cursive', 'coachpress-lite' ),
		'impact-charcoal'     => __( 'Impact, Charcoal', 'coachpress-lite' ),
        'lucida'              => __( 'Lucida Sans Unicode, Lucida Grande', 'coachpress-lite' ),
        'tahoma-geneva'       => __( 'Tahoma, Geneva', 'coachpress-lite' ),
		'trebuchet-helvetica' => __( 'Trebuchet MS, Helvetica', 'coachpress-lite' ),
		'verdana-geneva'      => __( 'Verdana, Geneva', 'coachpress-lite' ),
        'courier'             => __( 'Courier New, Courier', 'coachpress-lite' ),
        'lucida-monaco'       => __( 'Lucida Console, Monaco', 'coachpress-lite' ),
	) );
