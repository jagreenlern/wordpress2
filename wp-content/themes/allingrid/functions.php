 ...
 */
define( 'ALLINGRID_MIN_PHP_VERSION', '5.6' );

/**
 * Immediately after theme switch is fired we we want to check php version and
 * revert to previously active theme if version is below our minimum.
 */
add_action( 'after_switch_theme', 'allingrid_test_for_min_php' );

/**
 * Switches back to the previous theme if the minimum PHP version is not met.
 */
function allingrid_test_for_min_php() {

	// Compare versions.
	if ( version_compare( PHP_VERSION, ALLINGRID_MIN_PHP_VERSION, '<' ) ) {
		// Site doesn't meet themes min php requirements, add notice...
		add_action( 'admin_notices', 'allingrid_min_php_not_met_notice' );
 ...
 */
function allingrid_min_php_not_met_notice() {
	?>
	<div class="notice notice-error is_dismissable">
		<p>
			<?php esc_html_e( 'You need to update your PHP version to run this theme.', 'allingrid' ); ?> <br />
			<?php
			printf(
				/* translators: 1 is the current PHP version string, 2 is the minmum supported php version string of the theme */
				esc_html__( 'Actual version is: %1$s, required version is: %2$s.', 'allingrid' ),
				PHP_VERSION,
				ALLINGRID_MIN_PHP_VERSION
			); // phpcs: XSS ok.
			?>
		</p>
	</div>
...
if ( ! function_exists( 'allingrid_fonts_url' ) ) :
	...
	function allingrid_fonts_url() {
	    ...
	    */
	    $questrial = _x( 'on', 'Lato, Poppins and PT Serif font: on or off', 'allingrid' );

	    if ( 'off' !== $questrial ) {
	        $font_families = array();
	 
	        $font_families[] = 'Lato:300,400,400i,700|Poppins:300,400,500,600,700|PT+Serif:400,400';
	...
	public function sections( $manager ) {
		...
		$manager->add_section(
			new allingrid_Customize_Section_Pro(
				...
				'allingrid',
				array(
					'title'    => esc_html__( 'ALPHANA', 'allingrid' ),
					'pro_text' => esc_html__( 'Upgrade to Pro', 'allingrid' ),
					'pro_url'  => esc_url( 'https://tishonator.com/product/alphana/' )
				)
			)
		);
	}
...
if ( ! function_exists( 'allingrid_sanitize_email' ) ) :
	function allingrid_sanitize_email( $email, $setting ) {
...
endif; // End of allingrid_sanitize_email

if ( ! function_exists( 'allingrid_sanitize_url' ) ) :
	function allingrid_sanitize_url( $url ) {
		return esc_url_raw( $url );
	}
endif; // End of allingrid_sanitize_url
 ...
 */
function allingrid_skip_link_focus_fix() {
...
add_action( 'wp_print_footer_scripts', 'allingrid_skip_link_focus_fix' );