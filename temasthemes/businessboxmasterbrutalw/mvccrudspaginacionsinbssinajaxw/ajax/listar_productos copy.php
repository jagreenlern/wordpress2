...
if($action == 'ajax'){
	$query = mysqli_real_escape_string($imodel->con,(strip_tags($_REQUEST['query'], ENT_QUOTES)));
	...
	$count_query   = mysqli_query($imodel->con,"SELECT count(*) AS numrows FROM $tables where $sWhere ");
	if ($row= mysqli_fetch_array($count_query)){$numrows = $row['numrows'];}
	else {echo mysqli_error($imodel->con);}
	$total_pages = ceil($numrows/$per_page);
	//main query to fetch the data
	$query = mysqli_query($imodel->con,"SELECT $campos FROM  $tables where $sWhere LIMIT $offset,$per_page");
	...
	?>
		<div class="table-responsive">
			<table class="table table-striped table-hover">
				...
				<tbody>	
						<?php 
					$finales=$imodel->mostrartabla($query);
